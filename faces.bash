...
for CHARA_ID in "$1"/*; do
    for FILE in "$CHARA_ID/"+([^_]).png; do
        if [[ "x$(echo ${FILE%.png}_*.png)" == "x" ]] ; then
            ...
            convert $FILE -gravity South-East -background white -splice 1x0 -background black -splice 1x0 -trim -chop 1x0 -background white -splice 0x1 -background black -splice 0x1 -trim -chop 0x1 $(dirname $FILE)/__tmp_$(basename $FILE)
